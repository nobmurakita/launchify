...
func (m *formModel) buildFields() []Field {
	...
	return []Field{
		...
		NewSelectField("スケジュール", []SelectOption{
			...
			WithDrillDownFunc(m.scheduleOnDrillDownCmd),
		...
		// 11. アクション選択
		NewConfirmField("アクション", "", &s.skipPreview,
...
func (m formModel) handleAdvance(drillDown bool) (formModel, tea.Cmd) {
	...
	if drillDown {
		// ドリルダウン遷移を判定
		if cmd := m.checkDrillDown(visible); cmd != nil {
			return m, cmd
		}
		// ボタンフィールドなら送信
...
func (m formModel) checkDrillDown(visible []Field) tea.Cmd {
	if m.focused >= len(visible) {
		return nil
	}
	f := visible[m.focused]
