...
2. **実行条件** — RunAtLoad、KeepAlive、スケジュール（Interval / Calendar）
3. **環境変数** — `KEY=VALUE` 形式で指定
4. **ログ出力** — ファイル出力（初期値: `~/Library/Logs/<Label>.log`）
...
- **ログファイル出力** — stdout/stderrを指定パスに出力。初期値は `~/Library/Logs/<Label>.log`
